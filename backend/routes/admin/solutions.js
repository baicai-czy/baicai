...
router.post('/', requirePermission('solutions:manage'), async (req, res, next) => {
  try {
...
router.put('/:id', requirePermission('solutions:manage'), async (req, res, next) => {
  try {
    const { title, targetCustomer, description, imageUrl, detail, isActive } = req.body
    await pool.query(
...
router.delete('/:id', requirePermission('solutions:manage'), async (req, res, next) => {
  try {
