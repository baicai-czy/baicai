...
function openDialog(item?: NewsItem) {
  if (item) {
    ...
    dialogTitle.value = '新建新闻'
    form.value = { title: '', category: 'company' as NewsCategory, summary: '', content: '', isPinned: false, isPublished: false, tags: [] }
  }
    ...
    <el-table :data="list" v-loading="loading" stripe border style="width: 100%">
      ...
      >
        <template v-if="col.prop === 'isPinned'" #default="{ row }">
          <el-tag :type="row.isPinned ? 'warning' : 'info'" size="small">
            {{ row.isPinned ? '是' : '否' }}
          </el-tag>
        </template>
        ...
        <template v-else-if="col.prop === 'category'" #default="{ row }">
          <el-tag size="small">
            {{ { company: '公司新闻', industry: '行业动态', notice: '通知公告' }[row.category] || row.category }}
          </el-tag>
        </template>
      </el-table-column>
