...
| # | 修复项 | 说明 |
|---|--------|------|
| 1 | 组织架构 | 接入OrgChart组件，替代简陋div树，支持连线/头像/点击详情弹窗 |
| 2 | RBAC权限 | 路由守卫加权限校验+菜单按角色过滤(12权限)，超级管理员'*'全部通过 |
| 3 | 图片fallback | NewsCard/PartnerWall/SolutionCard等所有img标签加@error隐藏处理 |
| 4 | 集成页案例 | 新增3个成功案例(CaseCard)，SOW要求的案例展示板块 |
| 5 | 国资云咨询 | 新增'立即咨询'CTA按钮，引导用户直达联系我们 |
| 6 | 字体 | 思源黑体本地回退+CDN双保障 |
| 7 | 地图 | 已预留SDK接口，低优先级后续集成 |

- 分支: fix/v1.8.6 (等待审核后合并到main)
...
- 修复首页无法跳转：恢复 vite.config.ts 中 /uploads 代理配置
...
**日期**：2026-07-30
- 网关错误根治：Axios 请求拦截器统一检查 VITE_USE_API，false 时 AbortController 直接中断
- 修复管理平台重复"用户管理"菜单和路由
